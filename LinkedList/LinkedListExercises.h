...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (!head_){
    ...
    traverser = head_;
    while (traverser->next && !added){
      if((traverser->data) >= (newNode->data))
      {
        if (traverser == head_){
          head_=newNode;
          newNode->next=traverser;
          traverser->prev=newNode;
        } else if (traverser == tail_){
          tail_ = newNode;
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // -----------------------------------------------------------
  Node* cur=left.head_;
  Node* othercur= right.head_;

  while (cur&&othercur) {
    //push smaller number to merged
    if (cur->data < othercur->data){
      merged.pushBack(cur->data);

      if (cur->next)
      //traverse
        cur= cur->next;
      else if(cur==left.tail_)
        cur=nullptr;
    }
    else {
      //push other smaller or equal number to merged
      merged.pushBack(othercur->data);
      //make sure pointer is not terminated or not a tail (has nex)
      if(othercur->next){
      //traverse
      othercur= othercur->next;
      }
      //if tail, push the last element then terminate, this ends the loop
      else if(othercur==right.tail_){
        othercur=nullptr;
      }
    }
  }
 //laborer is going to stack the rest of the left over LinkedList
  Node *laborer = cur? cur: othercur;
  while (laborer){
    merged.pushBack(laborer->data);
    if (laborer->next)
    //traverse
      laborer= laborer->next;
    else
      laborer=nullptr;
  }
  // -----------------------------------------------------------
