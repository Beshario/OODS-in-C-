...
static void treeFactory(GenericTree<int>& tree) {
  ...
  root=tree.createRoot(4);
  auto child=root->addChild(8);
  root->addChild(15);
  auto next=child->addChild(16);
  child->addChild(23);
  next->addChild(42);
  ...
  // hen you print it out. The main() function runs that test for you.
  // ...
}
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  while(!nodesToExplore.empty())
  {
    auto frontNode=nodesToExplore.front();
    nodesToExplore.pop();
    if (!frontNode) {
      // If the top node pointer is null, then we must not dereference it.
      // Just increment the null counter, then "continue" to jump back to the top of the loop.
